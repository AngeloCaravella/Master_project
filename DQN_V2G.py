...
DQN_PARAMS = {
    ...
    'early_stopping_patience': 100 # N. di controlli (x100 episodi) senza miglioramenti
}
...
def run_training(agent: DQNAgent, env: V2G_Environment, model_path: str):
    ...
    for episode in range(num_episodes):
        ...
        agent.decay_epsilon()
        if (episode + 1) % DQN_PARAMS['target_update_freq'] == 0: agent.update_target_network()
        ...
        if (episode + 1) % 100 == 0:
            ...
            if avg_reward > best_avg_reward:
                ...
                agent.save_model(model_path) # Salva il modello migliore trovato finora
                patience_counter = 0
            else:
                patience_counter += 1
